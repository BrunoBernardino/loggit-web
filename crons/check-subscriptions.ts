...
async function checkExpiredTrials() {
  ...
  try {
    const users = await db.query<User>(
      sql`SELECT * FROM "loggit_users" WHERE "status" = 'trial' AND "subscription" ->> 'expires_at' <= $1`,
      [
        now,
      ],
    );
    ...
    for (const user of users) {
      ...
      const userSessions = await db.query<Pick<UserSession, 'id'>>(
        sql`SELECT * FROM "loggit_user_sessions" WHERE "user_id" = $1 AND "last_seen_at" >= $2`,
        [
          user.id,
          sevenDaysAgo,
        ],
...
async function checkExpiredSubscriptions() {
  ...
  try {
    const users = await db.query<User>(
      sql`SELECT * FROM "loggit_users" WHERE "status" = 'active' AND "subscription" ->> 'expires_at' <= $1 AND "subscription" ->> 'updated_at' <= $1`,
      [
        threeDaysAgo,
      ],
    );
