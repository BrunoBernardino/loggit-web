...
async function cleanupSessions() {
  ...
  try {
    const result = await db.query<{ count: number }>(
      sql`WITH "deleted" AS (
        DELETE FROM "loggit_user_sessions" WHERE "expires_at" <= $1 RETURNING *
      )
...
async function cleanupInactiveUsers() {
  ...
  try {
    const result = await db.query<Pick<User, 'id'>>(
      sql`SELECT "id" FROM "loggit_users" WHERE "status" IN ('inactive', 'trial') AND "subscription" ->> 'expires_at' <= $1`,
      [
